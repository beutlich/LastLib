...
fmiStatus fmiSetReal(fmiComponent c, const fmiValueReference vr[], size_t nvr, const fmiReal value[]){
    ...
    for (i=0; i<nvr; i++) {
       if (vrOutOfRange(comp, "fmiSetReal", vr[i], NUMBER_OF_REALS))
           return fmiError;
       if (comp->loggingOn) comp->functions.logger(c, comp->instanceName, fmiOK, "log",
           "fmiSetReal: %s = %.16g", realVarNames[vr[i]], value[i]);
       comp->r[vr[i]] = value[i];
    }
    return fmiOK;
}
...
fmiStatus fmiGetReal(fmiComponent c, const fmiValueReference vr[], size_t nvr, fmiReal value[]) {
    ...
    for (i=0; i<nvr; i++) {
        ...
        if (comp->loggingOn) comp->functions.logger(c, comp->instanceName, fmiOK, "log",
                "fmiGetReal: %s = %.16g", realVarNames[vr[i]], value[i]);
    }
#endif
    return fmiOK;
}
...
fmiStatus fmiCompletedIntegratorStep(fmiComponent c, fmiBoolean* callEventUpdate){
    ...
    completedIntegratorStep(comp); // to be implemented by the includer of this file
