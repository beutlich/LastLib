...
model Last_dymola_linux "Last for Dymola on Linux"
...
void * Last_fmiInstantiateModel2(const char*instanceName, fmiBoolean loggingOn) {
  ...
  if (res!=0) {
    if (!(res->hInst=LoadLibrary(\"Last.so\"))) {
      ModelicaError(\"Loading of FMU dynamic link library (Last.so) failed!\");
      return 0;
    }
...
#endif", Library="Last", LibraryDirectory="modelica://LastLib/Resources/binaries",__Dymola_doNotLinkSharedObject=true, __Dymola_CriticalRegion="Last");
    end constructor;
...
#endif", Library="Last", LibraryDirectory="modelica://LastLib/Resources/binaries",__Dymola_doNotLinkSharedObject=true, __Dymola_CriticalRegion="Last",__Dymola_IdemPotent=true, __Dymola_VectorizedExceptFirst=true);
    end fmiSetReal;
